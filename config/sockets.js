...
module.exports.sockets = {
  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
    ...
    /** Need to check if student is in chatroom before watching
    * need to avoid messages being published to all chatrooms.
    */

    //maybe check the chatroo
    // console.log("User: "+session.Student);
    Chatroom.watch(socket);
  },
  ...
  ***************************************************************************/
  onDisconnect: function(session, socket) {
    ...
    Student.find({id:session.Student.id}).populateAll()
    .exec(function session(err, student){
      if(student.chatroom) {
      student[0].remove(student.chatroom.id);
      student[0].save(function disconnected(err, stdnt) {
        if(err) return err;
